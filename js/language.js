...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setLanguage(lang) {
        ...
        // Update toggle button text - show the *other* language option
        const labelSpan = langToggle.querySelector('span');
        if (labelSpan) {
            labelSpan.textContent = lang === 'en' ? 'ಕನ್ನಡ' : 'English';
        }
        
        // Add active state styling classes if needed
        if (lang === 'kn') {
            langToggle.classList.add('is-kn');
        } else {
            langToggle.classList.remove('is-kn');
        }

        // Update all translatable elements having [data-lang-key]
